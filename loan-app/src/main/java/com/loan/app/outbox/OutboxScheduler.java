...
public class OutboxScheduler {
    ...
    private void doScanAndRetry() {
        ...
        for (OutboxMessage message : pendingMessages) {
            try {
                ...
                log.info("【Outbox调度】消息发送成功: messageId={}", message.getMessageId());

            } catch (Exception e) {
                log.error("【Outbox调度】消息发送失败: messageId={}, error={}",
    ...
    public void trySendImmediately(OutboxMessage message) {
        try {
            ...
            outboxRepository.updateStatus(message.getId(), OutboxMessage.STATUS_SENT);
            log.info("【Outbox快速通道】消息发送成功: messageId={}", message.getMessageId());
        } catch (Exception e) {
            log.warn("【Outbox快速通道】消息发送失败，等待调度器补偿: messageId={}", message.getMessageId());
            // 不更新状态，调度器会处理
        }
    }
}